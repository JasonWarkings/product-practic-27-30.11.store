    ...
    <div class="row mb-4">
      <div class="col-md-4">
        <select class="form-select" v-model="category" @change="filterCategory">
          <option value="" selected>Select a categori</option>
          <option v-for="category in categoryStore.categories" :value="category" :key="category">{{ category }}</option>
        </select>
      </div>
      <div class="col-md-6">
        <input type="text" class="form-control" v-model="searchText" @input="searchName" placeholder="Name product" />
      </div>
      <div class="col-md-2">
        <button class="btn btn-outline-danger w-100" @click="resetFilters">Delete</button>
      </div>
    </div>
